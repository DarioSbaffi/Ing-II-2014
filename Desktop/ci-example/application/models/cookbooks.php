...
class Cookbooks extends CI_Model{
	...
	public function updateAutor($dat,$id){
		...
		if($query->num_rows()==0){
			...
			?><script language="javascript">alert("Se a modificado el Autores correctamente");window.location.href="http://localhost/ci-example/abm/Autores/";</script><?
		}
		else{
			?><script language="javascript">alert("No se ha logrado insertar el autor debido a que ya existe");window.location.href="http://localhost/ci-example/abm/Autores/";</script><?
		}
	}
