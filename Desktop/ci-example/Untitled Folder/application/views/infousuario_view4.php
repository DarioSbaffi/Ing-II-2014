				...
				<div class="container">
					...
					<?if(!$resultados) {
						...
						<table class="table table-bordered table-hover" id="catalogo">
							<tr><td>Nombre de Usuario</td><td>Nombre</td><td>Apellido</td></tr>
							<?foreach($resultados as $fila){?>
								<tr><td><?=$fila->nom_usuario?></td><td><?=$fila->nombre?></td><td><?=$fila->apellido?></td><td><button onclick="if(confirm('Esta seguro de realizar la baja?')){window.location='<?=base_url()?>info_usuario/baja_logica/<?=$fila->id_usuario?>'}">Realizar Baja</button>
								</td></tr>
							<?}}?> 
						</table>
				</div>
