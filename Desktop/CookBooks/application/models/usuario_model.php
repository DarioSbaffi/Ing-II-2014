...
class Usuario_model extends CI_Model {
    ...
    function info_perfil($id) {
		$consulta=$this->db->select('id_usuario,dni, usuario.nombre as nom, apellido, correo_electronico,
									...
									ciudad.id as idciudad,ciudad.nombre as nom_ciudad,
									ciudad.id_provincia as idp')
							->from('usuario')
    ...
    function pedido2() {
		$consulta=$this->db->select('pedido.id_pedido as id,usuario.nom_usuario,libro.titulo as titulol,fecha,pedido.estado as ped')
							->from('pedido')
							->join('usuario','pedido.id_usuario=usuario.id_usuario')
							->join('libro_pedido','pedido.id_pedido=libro_pedido.id_pedido')
							->join('libro','libro_pedido.id_libro=libro.id_libro')
							->where_not_in('pedido.estado','finalizado');
        $consulta = $this->db->get('');
        if ($consulta->num_rows() > 0) {
            return $consulta->result();
